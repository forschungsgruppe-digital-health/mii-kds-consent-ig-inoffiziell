# Changelog - MII Implementation Guide Consent v2026.0.1
...
The changelog below is the module's own, as the published implementation guide carries it. It ends at version 2026.0.0; the conformance resources migrated in this repository correspond to the development state (`develop`, prerelease 2026.0.1-rc-4), for which no published guide exists — see the migration report's decision queue.

> [TODO:REVIEW — Gate C. The release-note entries below are the module's own German text. They are a historical record of the published releases; a translation is a review decision, not a mechanical one, so they are carried over verbatim.]

### Änderungshistorie

Die folgende Änderungshistorie ist die des Moduls, wie der publizierte Implementierungsleitfaden sie führt. Sie endet bei Version 2026.0.0; die in diesem Repository migrierten Konformitätsressourcen entsprechen dem Entwicklungsstand (`develop`, Vorabversion 2026.0.1-rc-4), für den noch kein publizierter Leitfaden vorliegt — siehe Entscheidungsliste des Migrationsberichts.
