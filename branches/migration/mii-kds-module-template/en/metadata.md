# Metadata Overview - MII Implementation Guide Consent v2026.0.0
...
**Migration note (2026-08-06):** the source guide has no metadata page; this page describes this repository's CRMI metadata. The `[TODO ...]` prompts below are **template setup items for the module owners**, not missing narrative.

This page documents the computable metadata of the **Consent** module. The metadata exists so that the artifacts of this guide can be discovered, evaluated, validated, cited, governed and reused — by people and by software alike.
...
The following CRMI-related metadata is set in [`sushi-config.yaml`](https://github.com/forschungsgruppe-digital-health/mii-kds-consent-ig-inoffiziell/blob/main/sushi-config.yaml) and ends up in the generated `ImplementationGuide` resource.
...
| [Resource Approval Date](http://hl7.org/fhir/extensions/5.3.0/StructureDefinition-resource-approvalDate.html) | Artifact lifecycle; publishing; governance | `ImplementationGuide.extension`(`2026-08-06`) | Records the date on which the publisher officially approved the content for use. |
