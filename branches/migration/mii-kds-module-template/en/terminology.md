# Terminology - MII Implementation Guide Consent v2026.0.1
...
This module defines three value sets and three code systems. The **tables below are carried over verbatim from the German source page**: they are the code systems' and value sets' own content — codes, OIDs and the displays the publisher assigns them — and translating a published display would falsify the terminology rather than translate it. The explanatory prose around them is translated; where it is not, the German paragraph is left in place and marked.

> [TODO:REVIEW — Gate C. The explanatory notes interleaved with the tables below are still the German source text. They are content of the published module and a translation of them is a review decision, not a mechanical one.]
